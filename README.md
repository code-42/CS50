...
About CS50
To read more in-depth about what changes we’ve introduced to CS50 this fall, visit cs50.ly/new.
...
Function in Scratch
Back in the day, David created his own game when he discovered Scratch, and now we have a modern recreation of it. Check that out, among other examples, here.
...
We can drag a few blocks to have Scratch say something:

Hello, world of Scratch
We can experiment with other blocks like play sound and wait to have Scratch meow.

And to have it repeat, instead of dragging the same blocks in over and over again, we can use the repeat block:

Meow
With a little more experimentation, we can have Scratch go back and forth on the screen and even say "ouch":

Ouch
Now we’ll demonstrate how to use a variable:
...
We can do something a little more interactive, where Scratch will say "meow" when the cursor touches it.

Touching
And a variation where it doesn’t …​

A thread is just a series of things that a computer is doing in order, so multiple threads mean that multiple series of things being done at the same time. In threads, we have two characters, each of which have a series of blocks associated with them, that both start with a when green flag clicked. So when we click the green flag, both characters will start moving simultaneously.

And another example has two threads for the same character, where the seal continues to bark until the muted variable is set to 1, or true. Both threads are checking for either the space key being pressed or the value of the muted variable, over and over again.

And finally, someone went through a lot of effort to implement Pokemon Go in Scratch, and there are lots of building blocks but none much fancier than what we’ve already played with.

If we look at another version of the program we started with, we can see how we can make a custom cough puzzle piece:
...
We can go even further and have cough itself take in some argument for the number of times to cough, and now we can simply call that block and tell it how many times to cough:
...
Finally, we take a look at what awaits us in CS50.